    ...
    class OutputFolderHelper
    {
        public static void SaveOutputFolder(string folder)
        {
            ...
            */
            ...
            for (int k = m; k < cmbc; k++)
            {
                frmMain.Instance.cmbOutputDir.Items.RemoveAt(m);
            }

            for (int k = 0; k < m && k<lst.Count; k++)
            {
                frmMain.Instance.cmbOutputDir.Items.Insert(m + k, lst[k]);
            ...
            frmMain.Instance.cmbOutputDir.SelectedIndex = m;
        }
